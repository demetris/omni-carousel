...
1.  Index maps & direct array access
    -   slideIndexMap: Maps DOM elements to their indexes
    -   Direct array access via `slides[index]`
    -   Indicators array with direct index access

2.  Mathematical index calculations
    -   Adjacent item: anchorIndex ± 1
...
5.  Smart navigation strategies
    -   Fast path: When one item visible or cached center index valid, use math
    -   Slow path: Only when necessary, calculate positions

6.  Utility functions that cache
...
9.  Optimized multi-step navigation
...
Navigate as much as the layout allows without querying the DOM.
When the DOM is first queried, cache everything that can help in query-less navigation.
Keep updating the cache as appropriate while navigating.
